...
if [ "$1" == "1" ]; then
	...
	coPatReporter.rb -i $dataset -o $current/results/cohortAn/pmm2 -p phenotypes -c chr -d patient_id -C 25 -s start -e stop -S ',' -m lin -T 2 -D
fi
...
if [ "$1" == "2" ]; then
	...
	kernelsPATH=/mnt/scratch/users/bio_267_uma/federogc/executions/backupgenes/similarity_kernels
	#datasets=( '2016' '2020' )
	datasets=( '2020' )
	...
	do
		if [[ "$dataset" == '2016'  ]]; then
			...
			target=$targetPath/mRNAseq_$dataset/analysis/DEG_workflow/TARGETS/CTL_vs_MUT_bicor_target.txt
			aux_target=$targetPath/mRNAseq_$dataset/analysis/DEG_workflow/CTL_vs_MUT_bicor_target.aux

		fi
		var_info=`echo -e "\\$target=$target,
			\\$aux_target=$aux_target,
			\\$packages=$packages,
			\\$simKernHGNClstnpy=$kernelsPATH/gene_hgncGroup/if_0001/kernel_matrix_bin.npy,
			\\$simKernHGNClst=$kernelsPATH/gene_hgncGroup/if_0001/kernel_matrix_bin.lst,
			\\$simKernPPIlstnpy=$kernelsPATH/protein_interaction/if_0001/kernel_matrix_bin.npy,
			\\$simKernPPIlst=$kernelsPATH/protein_interaction/if_0001/kernel_matrix_bin.lst,
